...
# -----------------------------
# [핵심] 이미지 로딩 최적화 - 메모리 렉 방지
# -----------------------------
@st.cache_resource
def get_cached_image(img_path):
    """이미지를 한 번 읽으면 메모리에 저장해두고 재사용합니다."""
    try:
...
def safe_show_image(path_or_url, caption=None):
    """경고 로그 생성을 막고 최신 규격으로 이미지를 출력합니다."""
    if path_or_url:
        img = get_cached_image(path_or_url)
        if img:
            # use_container_width=True 대신 width='stretch'를 사용하여 렉 유발 로그 차단
...
if st.session_state.done and st.session_state.result:
    ...
    if st.button("🔁 테스트 다시하기", width='stretch'):
        reset()
        st.rerun()
    ...
    for key, text in q_obj["opts"].items():
        if st.button(f"{text}", key=f"btn_{q_idx}_{key}", width='stretch'):
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("⬅️ 뒤로", width='stretch', disabled=(q_idx == 0)):
            if st.session_state.answers:
                st.session_state.answers.pop()
            st.session_state.q_idx = max(0, st.session_state.q_idx - 1)
            st.rerun()
    with c2:
        if st.button("🗑️ 처음부터", width='stretch'):
            reset()
            st.rerun()
